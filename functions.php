...
function disable_product_links_for_selected_categories() {
    ...
    // Categories where we disable links
    $categories_to_disable = array('tray', 'chocolates', 'packaging', 'accessories');

    // Check if product belongs to the categories
    if (has_term($categories_to_disable, 'product_cat', $product->get_id())) {
...
add_action('woocommerce_after_shop_loop_item', 'add_select_tray_button', 10);

// Replace add to cart button with "Select Chocolate" for Chocolates
function add_select_chocolate_button() {
    global $product;

    if (has_term('chocolates', 'product_cat', $product->get_id())) {
        $product_id = $product->get_id();
        echo '<button class="select-chocolate-button" data-id="' . esc_attr($product_id) . '">
                Select This Chocolate
              </button>';
    }
}
remove_action('woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10);
...
//     if (!isset($_GET['tray'])) {
//         return 'No tray product ID provided. abcd';
//     }
...
//         $cavities[$match[1]] = intval($match[2]);
//     }
...
//             if (isset($cavities[$fits_cavity])) {
//                 $chocolate_counts[$fits_cavity][] = $chocolate->get_name();
//             }
//         }
//     }
...
//         $output .= '<li>Cavity Size: ' . esc_html($size) . ' - Quantity: ' . esc_html($quantity);
//         if (!empty($chocolate_counts[$size])) {
//             $output .= '<ul>';
//             foreach ($chocolate_counts[$size] as $choco_name) {
//                 $output .= '<li>Chocolate: ' . esc_html($choco_name) . '</li>';
//             }
//             $output .= '</ul>';
//         } else {
//             $output .= '<ul><li>No chocolates found for this cavity size.</li></ul>';
//         }
//         $output .= '</li>';
//     }
//     $output .= '</ul>';
